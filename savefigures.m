...
colorct = {'k','w','r','y','c','g'};
plotct = {'-','.','--','-.','.-'};
d4list = {'RMSSDj','SDNNj','pNN50j','Averagej'};
...
lsf = uicontrol('Style','listbox','Position',[10,40,170,200],'String',...
    ...
    'set(p1sv,''Enable'',''off''); set(p2sv,''Enable'',''off'');set(p4sv,''Visible'',''off'');',...
    'set(p3sv,''Enable'',''on'');elseif strcmp(options{get(lsf,''Value'')},',...
    '''STAR'');set(p1sv,''Enable'',''off'');set(p3sv,''Enable'',''on'');set(p4sv,''Visible'',''off'');',...
    'set(p2sv,''Enable'',''off'');elseif strcmp(options{get(lsf,''Value'')},',...
    '''Time-Varying'');set(p3sv,''Enable'',''off'');set(p4sv,''Visible'',''on'');',...
    'else;set(p1sv,''Enable'',''on'');',...
    'set(p2sv,''Enable'',''on'');set(p3sv,''Enable'',''off'');',...
    'set(p4sv,''Visible'',''off'');end'] );
...
p3sv = uicontrol('Style','pop','String',d3list,'position',...
    [200,130,70,40]);
p4sv = uicontrol('Style','pop','String',d4list,'position',...
    [200,100,70,40],'Visible','off');
pltype = 'k-';
...
bsf1 = uicontrol('Position',[10,15,120,20],'String', 'Preview',...
    ...
    'ylabel(''Frequency (Hz)'');title(''Short-Time Auto Regressive'');elseif ',...
    'strcmp(gopt,''Time-Varying''); gopt1 = d4list{get(p4sv,''Value'')};',...
    'if strcmp(gopt1,''RMSSDj'');plot(Index,RMSSDj,pltype);ylabel(''RMSSDj (ms)'')',...
    ';elseif strcmp(gopt1,''SDNNj'');',...
    'plot(Index,SDNNj,pltype);ylabel(''SDNNj (ms)'');elseif ',...
    'strcmp(gopt1,''pNN50j'');plot(Index,pNN50j,pltype);ylabel(''pNN50 (%)'');',...
    'else;plot(Index,averagej,pltype);ylabel(''Averagej (ms)'')',...
    ';end; xlabel(''Time (s)'');axis(''tight'');end;']);
...
bsf2 = uicontrol('String','Save','Callback',['gopt = options(get(lsf,''Value''));',...
    ...
    'elseif strcmp(gopt,''ECG''); plot(ecg_time,ECG,pltype);xlabel(''Time (s)'');',...
    'ylabel(''V''); title(''Electrocardiogram'');elseif strcmp(gopt,''STFT'');',...
    'if get(p3sv,''Value'') == 1 ;imagesc(Time,Fft,Pft);else;surf(time_stft,',...
    'Fft,Pft);zlabel(''PSD (ms^2/Hz)'');shading interp;end;xlabel(''Time (s)'');ylabel(''Frequency (Hz)'');',...
    'title(''Short-Time Fourier Transform'');elseif strcmp(gopt,''STAR'');',...
    'if get(p3sv,''Value'') == 1;imagesc(Time,Fft_AR,Pft_AR);else;surf(time_star,',...
    'Fft_AR,Pft_AR);zlabel(''PSD (ms^2/Hz)'');shading interp;end;xlabel(''Time (s)'');ylabel(''Frequency (Hz)'');',...
